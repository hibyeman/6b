...
if (queue.length === 0) { // if there are no songs in the queue, send message that queue is empty
	...
	if (args.length == 2 && args[1] <= queue.length) { // check if there are no more than 2 arguments and that second one is in range of songs number in queue
		...
		message.reply(`**${songsQueue[args[1] - 1]}** has been removed from the queue. Type 6queue to see the current queue.`);
	...
	if (args.length == 1) {
		...
		message.reply("**You need to type 6queue clear without following arguments.**");
	}
	...
	message.channel.send("Queue has been shuffled. Type 6queue to see the new queue!");
